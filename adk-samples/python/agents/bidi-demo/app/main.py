...
from fastapi.responses import FileResponse, Response   # ← Added Response for favicon
...
# ========================================
os.environ.setdefault("GOOGLE_GENAI_TIMEOUT_SECONDS", "120")
app = FastAPI()
    ...
    async def upstream_task() -> None:
        ...
        while True:
            ...
            # Handle text frames (JSON messages)
                ...
                # Handle video frame with invisible Frame Intelligence metadata
                    ...
                    # The client analyzed the frame and injected structured metadata.
                    # We send this as a text context BEFORE the image so the model
                    # reads the frame stats first, then sees the actual pixels.
                    # This gives the model "superpowers" — it knows brightness,
                    # composition quality, dominant colors, and tilt BEFORE looking.
                    frame_intelligence = json_message.get("frameIntelligence")
                    if frame_intelligence:
                        logger.debug(f"Frame intelligence: {frame_intelligence[:120]}")
                        intelligence_content = types.Content(
                            parts=[types.Part(text=frame_intelligence)]
                        )
                        live_request_queue.send_content(intelligence_content)
                    # ──────────────────────────────────────────────────────────

                    logger.debug(
                        f"Sending video frame: {len(image_data)} bytes, "
                        f"type: {mime_type}"
                    )

                    # Send as realtime blob (this is how Gemini Live sees it)
                    video_blob = types.Blob(
                        mime_type=mime_type, data=image_data
                    )
                    live_request_queue.send_realtime(video_blob)
                
                # Handle single image capture (keep existing)
...
@app.websocket("/ws-video/{user_id}/{session_id}")
async def websocket_video_endpoint(
    websocket: WebSocket,
    user_id: str,
    session_id: str,
) -> None:
    """Dedicated WebSocket for video frames only.
    
    Keeps video frames off the audio pipeline entirely so voice
    interruptions are never delayed by large JPEG blobs.
    """
    await websocket.accept()
    logger.debug(f"Video WebSocket connected: session_id={session_id}")
    ...
    try:
        while True:
            ...
            if json_message.get("type") != "video_frame":
                continue

            # Look up the live queue created by the audio WebSocket
            queue = active_queues.get(session_id)
            if not queue:
                # Audio WebSocket not connected yet — silently drop frame
                logger.debug(f"No active queue for session {session_id}, dropping frame")
                continue

            image_data = base64.b64decode(json_message["data"])
            mime_type = json_message.get("mimeType", "image/jpeg")

            # Send frame intelligence text context before the image
            frame_intelligence = json_message.get("frameIntelligence")
            if frame_intelligence:
                intelligence_content = types.Content(
                    parts=[types.Part(text=frame_intelligence)]
                )
                queue.send_content(intelligence_content)

            video_blob = types.Blob(mime_type=mime_type, data=image_data)
            queue.send_realtime(video_blob)

            logger.debug(f"Video frame forwarded: {len(image_data)} bytes")

    except WebSocketDisconnect:
        logger.debug(f"Video WebSocket disconnected: session_id={session_id}")
    except Exception as e:
        logger.error(f"Video WebSocket error: {e}", exc_info=True)